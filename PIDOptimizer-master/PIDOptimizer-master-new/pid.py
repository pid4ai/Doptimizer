            ...
            for p in group['params']:
                ...
                if momentum != 0:
                    ...
                    if 'grad_buffer' not in param_state:
                        ...
                        D_buf = param_state['D_buffer'] = torch.zeros_like(p.data)
                        D_buf.mul_(momentum).add_((d_p.detach()/ (v_buf ** 0.5 + epsilon)) - g_buf)
                    else:
                        D_buf = param_state['D_buffer']
                        g_buf = param_state['grad_buffer']
                        D_buf.mul_(momentum).add_(1 - momentum, (d_p.detach()/ (v_buf ** 0.5 + epsilon)) - g_buf)
                        self.state[p]['grad_buffer'] = (d_p.detach()/ (v_buf ** 0.5 + epsilon)).clone()

                    d_p = ((d_p.add_(I, I_buf / (1 - momentum ** param_state['time_buffer']))) / (v_buf ** 0.5 + epsilon)).add_(D, D_buf)
                p.data.add_(-group['lr'], d_p)
