...
class D_Adamoptimizer(Optimizer):
    def __init__(self, params, lr=required, momentum=0.9, beta=0.99,
                 weight_decay=0,epsilon=0.0000001, I=1.):
        defaults = dict(lr=lr, momentum=momentum, beta=beta,
                        weight_decay=weight_decay, epsilon=epsilon, I=I)
        super(D_Adamoptimizer, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(D_Adamoptimizer, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                delta = (I_buf / (1 - momentum ** param_state['time_buffer'])) / (dv_buf ** 0.5 + epsilon)
                p.data.add_(-group['lr'], delta)
